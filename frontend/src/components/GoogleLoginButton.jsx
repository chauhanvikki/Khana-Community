...
export default function GoogleLoginButton({ onSuccess, onOTPRequired, role, disabled }) {
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      if (disabled) return;
      try {
        // Send the access_token to our backend, which will verify it with Google
  ...
  return (
    ...
    >
      <button
        ...
        onClick={() => googleLogin()}
        disabled={disabled}
        className="w-full flex items-center justify-center gap-3 px-6 py-3 bg-white border-2 border-gray-200 rounded-xl text-gray-700 font-semibold hover:bg-gray-50 hover:border-gray-300 hover:shadow-md transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
      >
