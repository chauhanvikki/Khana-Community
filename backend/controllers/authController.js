...
async function googleLogin(req, res) {
  ...
  try {
    ...
    try {
      ...
      console.error("OTP email sending failed:", mailErr.message, '| code:', mailErr.code);
      return res.status(500).json({ message: "Failed to send OTP email", error: mailErr.message, code: mailErr.code });
    }
...
async function resendOTP(req, res) {
  try {
    ...
    try {
      ...
      console.error("Mail sending failed:", mailErr.message, '| code:', mailErr.code);
      return res.status(500).json({ message: "Failed to send OTP email", error: mailErr.message, code: mailErr.code });
    }
